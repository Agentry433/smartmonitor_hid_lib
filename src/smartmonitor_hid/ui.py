...
from .theme import FontSpec, Geometry, SensorSpec, StartupPicSpec, StyleSpec, Theme, ThemeBundle, Widget, WidgetParent
...
def theme_to_xml(theme: Theme) -> str:
    root = ET.Element("ui")
...
def write_theme_file(path: str | Path, bundle_or_theme: ThemeBundle | Theme) -> None:
    theme = bundle_or_theme.theme if isinstance(bundle_or_theme, ThemeBundle) else bundle_or_theme
...
def parse_ui_xml(xml_text: str, path: str = "") -> Theme:
...
def parse_ui_file(path: str | Path) -> Theme:
    ui_path = Path(path)
...
def parse_theme_bundle(ui_path: str | Path) -> ThemeBundle:
