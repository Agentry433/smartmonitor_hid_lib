...
from .errors import SmartMonitorCompilerError
from .imgdat import SMARTMONITOR_RECORD_SLOT_SIZE, pack_record_fields, parse_imgdat_file
from .render import render_datetime_preview_payload, render_number_glyph_payload, render_static_text_payload
from .theme import ThemeBundle
from .ui import Theme, ThemeBundle, Widget, WidgetParent, detect_frame_count, parse_theme_bundle, resolve_theme_path, write_theme_file
...
def _default_donor_path(*candidates: str) -> Path:
    search_roots = [Path.cwd(), *Path.cwd().parents, PACKAGE_ROOT]
    for candidate in candidates:
        for root in search_roots:
            path = root / candidate
            if path.is_file():
                return path
    return PACKAGE_ROOT / candidates[0]
...
def _startup_record(bundle: ThemeBundle, offset: int) -> tuple[CompiledSmartMonitorRecord, bytes] | None:
...
def _background_record(bundle: ThemeBundle, parent: WidgetParent, offset: int) -> tuple[CompiledSmartMonitorRecord, bytes] | None:
    if not parent.background_image_path:
        return None
    width = parent.geometry.width
    height = parent.geometry.height
    payload, frame_count, is_png = _load_frame_payloads(bundle.base_dir, parent.background_image_path, width, height)
    record = CompiledSmartMonitorRecord(
        record_type=0x81,
        fields={
            "reserved_1": 0,
...
def _image_widget_record(bundle: ThemeBundle, widget: Widget, offset: int) -> tuple[CompiledSmartMonitorRecord, bytes]:
...
def _datetime_donor_payloads(bundle: ThemeBundle) -> dict[int, tuple[dict[str, Any], bytes]]:
...
def _datetime_format_preview(widget: Widget) -> str:
    fmt = (widget.datetime_format or "").lower()
    mapping = {"y": "1", "m": "2", "d": "3", "h": "4", "n": "5", "s": "6"}
...
def _datetime_widget_record(bundle: ThemeBundle, widget: Widget, offset: int) -> tuple[CompiledSmartMonitorRecord, bytes]:
...
def _static_text_record(widget: Widget, offset: int) -> tuple[CompiledSmartMonitorRecord, bytes]:
    ...
    rendered = render_static_text_payload(text, widget.font, vendor_mode=True, binary_threshold=160)
    font_color = widget.font.color if widget.font else 0
    record = CompiledSmartMonitorRecord(
        record_type=0x93,
        fields={
...
def compile_theme_bundle(bundle: ThemeBundle) -> bytes:
    ...
    allocation_priority = {0x94: 0, 0x81: 1, 0x84: 2, 0x93: 3, 0x92: 4, 0x8E: 4}
...
class ThemeCompiler:
    """Compiler facade for `.ui` -> `img.dat` work."""

    def compile_ui_file(self, ui_path: str | Path) -> bytes:
        try:
            return compile_theme_file(ui_path)
        except Exception as exc:
            raise SmartMonitorCompilerError(f"Failed to compile UI file: {ui_path}") from exc

    def compile_theme_bundle(self, bundle: ThemeBundle) -> bytes:
        from tempfile import NamedTemporaryFile

        with NamedTemporaryFile(suffix=".ui", delete=False) as temp_file:
            temp_path = Path(temp_file.name)
        try:
            write_theme_file(temp_path, bundle)
            return self.compile_ui_file(temp_path)
        finally:
            temp_path.unlink(missing_ok=True)

    def compile_ui_to_file(self, ui_path: str | Path, output_path: str | Path) -> Path:
        output_path = Path(output_path)
        output_path.write_bytes(self.compile_ui_file(ui_path))
        return output_path
